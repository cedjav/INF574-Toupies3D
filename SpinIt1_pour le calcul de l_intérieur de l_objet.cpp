...
int main(int argc, char *argv[]) {
	...
	int compteur = 0;
	MatrixXd NewV = MatrixXd(V.rows(), 3);
	for (int i = 0;i < V.rows();i++) {
		NewV.row(i) = V.row(i) - N_vertices.row(i)*epaisseur;
		if (((NewV(i, 1) - 14) * (V(i, 1) - 14) < 0) && (NewV(i, 2) - 14) * (V(i, 2) - 14) < 0) {
			//cout << "Vertice numero " << i << "Coordonnees du point avant : " << V(i, 0) << " - " << V(i, 1) << " - " << V(i, 2) << endl;
			//cout << "Normale numero " << i << "Coordonnees de la normale : " << N_vertices(i, 0) << " - " << N_vertices(i, 1) << " - " << N_vertices(i, 2) << endl;
			//cout << "Vertice numero " << i << "Coordonnees du point avant : " << NewV(i, 0) << " - " << NewV(i, 1) << " - " << NewV(i, 2) << endl;
			NewV(i, 1) = 14;
			NewV(i, 2) = 14;
			compteur++;
		}
	}
	cout << "Nombre de points qui disparaissent : "<< compteur;

	ofstream monFlux("../data/interieur_toupie_mod5_epaisseur15.off");

	monFlux << "OFF\n";
	monFlux << V.rows() << " " << F.rows() << " 0\n";
	for (int i = 0;i < V.rows();i++)
		monFlux << NewV(i, 0) << " " << NewV(i, 1) << " " << NewV(i, 2) << "\n";
	for (int i = 0;i < F.rows();i++)
		monFlux << "3 " << F(i, 0) << " " << F(i, 1) << " " << F(i, 2) << "\n";
	...
	for (int z = 0;z < MAX;z++) {
		for (int i = 0; i < MAX; i++) {
			for (int j = 0;j < MAX;j++) {
				if (SpaceRep.Space[i][j][z] == 2) cellbound++;
				if (SpaceRep.Space[i][j][z] == 1) cellint++;
			}
		}
	}
	...
	// Tentative d'utiliser le code Giblib : KO pour l'instant

	Eigen::VectorXi b;
	Eigen::VectorXd SZ,Diag;
	Eigen::VectorXd B, bc, lx, ux, Beq, Bieq, Z;
	Eigen::SparseMatrix<double> Q, Q1, Aeq, Aieq;
	MatrixXd Aeqcopy;

	// One fixed point for beta 0 set at 0.81 (to allow checking)
	b.resize(1, 1); 	b << 0; 	bc.resize(1, 1); 	bc << 0.81;

	// Zero linear term
	B = VectorXd::Zero(cellint, 1);
	SZ = VectorXd::Zero(cellint, 1);
	Diag = VectorXd::Zero(cellint, 1);

	// Lower and upper bound
	lx = VectorXd::Ones(cellint, 1);
	lx *= -1;
	ux = VectorXd::Ones(cellint, 1);
	ux *= 300;

	// Set the matrix Q and Aeq
	int cell = 0;
	Q = SparseMatrix<double>(cellint, cellint);
	Q1 = SparseMatrix<double>(cellint, cellint);
	Aeq = SparseMatrix<double>(4, cellint);
	Aeqcopy = MatrixXd(4, cellint);
	...
	double taille_voxel = taille_x * taille_y * taille_z;
	double masse = 7000; // A affiner plus tard!
	...
	for (int i=0; i<cellint; i++) {
		...
		double value = (gammaI * (xspace * xspace + yspace * yspace - 2 * zspace * zspace) * xspace) + (gammaC + gammaI / masse) * (zspace * zspace);
		SZ[i] = zspace * taille_voxel;
		Diag[i] = value;
		Q.insert(i, i) = value;
		if (i % 100 == 0) cout << "Dans la boucle 1 - ligne " << i << " sur " << cellint << endl;
		Aeq.insert(0, i) = xspace; Aeqcopy(0, i) = xspace;
		Aeq.insert(1, i) = yspace; Aeqcopy(1, i) = yspace;
		Aeq.insert(2, i) = xspace * zspace; Aeqcopy(2, i) = xspace * zspace;
		Aeq.insert(3, i) = yspace * zspace; Aeqcopy(3, i) = yspace * zspace;
		}
	for (int i = 0;i < cellint; i++) {
		for (int j = 0;j < cellint; j++)
			if (i!=j) Q.insert(i,j)= SZ[i] * SZ[j];
			else Q.coeffRef(i, j) += SZ[i] * SZ[j];
		if (i % 100 == 0) cout << "Dans la boucle 2 - ligne " << i << " sur " << cellint << endl;
	}


	Beq.resize(4, 1);
	Beq(0) = 0; Beq(1) = 0; Beq(2) = 0; Beq(3) = 0;
	...
	/*Eigen::VectorXd Beta;
	Beta = VectorXd::Ones(cellint);
	Beta = Beta * .5;

	double Energie = compute_energie(Beta, SpaceRep, gammaI, gammaC);
	*/
